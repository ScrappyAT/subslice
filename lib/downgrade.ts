...
export type ScheduleDowngradeResult =
  ...
  /** No planCode at all on this arm — not `null`. "Cannot determine" must
   * not be renderable as a plan name, and a nullable field would still
   * invite a fallback like `planCode ?? "free"` at the call site. See the
   * matching note on lib/cancellation.ts's PreviewCancellationResult. */
